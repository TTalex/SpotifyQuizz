app.controller('CategoryModeController',
    ['$scope', 'Spotify', '$sce', '$timeout', '$location', '$document',
    function ($scope, Spotify, $sce, $timeout, $location, $document) {
        ...
        $scope.selectplaylist = function(p) {
            ...
            Spotify.getPlaylistTracks(p.owner.id, p.id, { limit: 50 }).then(function (data) {
                handle_playlist_tracks(data);
                Spotify.getPlaylistTracks(p.owner.id, p.id, { limit: 50, offset: 50 }).then(function (data) {
                    handle_playlist_tracks(data);
                }, handle_err);
            }, handle_err);
        };
        ...
        $scope.choose = function(track) {
            ...
            if (fzS.get(track.id, null, 0.9)) {
                var sc = add_score();
                $scope.message = "Bravo ! +" + sc + " points.";
                $scope.responses.push(sc);
            } else {
                $scope.message = "Perdu ! La bonne réponse était " + $scope.random_track.artists[0].name + " - " + $scope.random_track.name;
                waitime = 5000;
